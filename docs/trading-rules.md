...
| 條件 | 說明 |
|------|------|
| AI 買進標籤 | 買單送出時使用 `custom_field=AIBUY`，成交後部位會標記 `owner_tag=AI`；沒有此標籤的手動/外部買進不會被本工具主動賣出 |
| 使用者指定門檻 | 若 `.env` 設定 `SELL_PROFIT_TARGETS=2330:8,0050:5.5`，該股票只有在 AI 部位報酬率達門檻後才允許自動賣出 |

有設定 `SELL_PROFIT_TARGETS` 的股票，達標時會直接送出賣單；未達標時，即使停損、移動停利或收盤全出場條件出現，也會被擋下並保留部位。
...
| `SELL_PROFIT_TARGETS` | 空 | 每檔使用者指定賣出門檻，例如 `2330:8,0050:5.5` |
