...
'切換下單模式 (等候下單回應echo與否)
Public Declare PtrSafe Function change_echo Lib "C:\vba dll\t4.dll" _
() As String
...
public Declare PtrSafe Function exam1st Lib "C:\vba dll\t4.dll" _
...
' 判斷字元是否為多字組(Multibyte Symbol)
Public Function LenMBS(ByVal ch As String) As Integer
    If ch="" Then
        LenMBS = 0
    ElseIf AscW(ch)>=128 Or AscW(ch)<0 Then
        LenMBS = 2
    Else
        LenMBS = 1
    End If
...
Public Function GetLengFromBytes(ByVal s As String, ByVal leng As Variant) As Variant
    ...
    Dim wch As Long       ' 用來判斷是 WCHAR 還是 CHAR
    Dim x As Long         ' 計算字元數
    ...
    lengB = 0
    Do While lengB < leng
        lengB = lengB + LenMBS(Mid(s, x+1, 1))
        x = x + 1
    Loop
